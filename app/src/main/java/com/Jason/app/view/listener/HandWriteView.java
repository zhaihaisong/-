...
import com.Jason.app.view.listener.point.DrawPoint;
import com.Jason.app.view.listener.point.PointUtil;
import com.Jason.app.view.listener.point.TimedPoint;
...
public class HandWriteView extends View {
    ...
    private Bitmap clearBlank(Bitmap bp, int blank) {
        ...
        //解决当字体过小时,字体被放大
        int i = dip2px(getContext(), 80);
        int width_y = right - left;//剪切的宽度
        int height_y = bottom - top;//剪切的高度

        if (width_y < i) {
            left = left - (i - width_y) / 2;//由于剪切的实际宽度被强制更改所以要设置剪切的实际位置  实现居中显示
            width_y = i;//剪切的宽度比80dp小时 强制宽度为80dp
        }
        if (height_y < i) {
            top = top - (i - height_y) / 2;//同上
            height_y = i;//同上

        }

        return Bitmap.createBitmap(bp, left, top, width_y, height_y);
    }
